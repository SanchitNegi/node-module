...
class UsersController extends AppController {
                ...
                if ($res['success']) {
                    ...
                    if ($result == 1) {
                        ...
                        if ($emailSuccess) {
                            ...
                            $url = HTTP_ROOT .'Users/accountActivation/'.$token;
                            $activationLink = '<a style="color:#fff;background-color: #10c4f7; text-decoration:none; padding: 5px 10px 7px;font-weight: bold; display:inline-block;" href="'.$url.'">Click here to activate your account</a>';
                        ...
                        $this->Session->setFlash(__('Registration successfully done, activation link has been sent to your email.'));
                        $this->redirect(array('controller' => 'users', 'action' => 'login'));
                    } else {
                        $this->Session->setFlash(__('Some problem has been occured in your registration process, please try again later.'));
                    }
                } else {
                    $this->Session->setFlash(__('Please re-enter reCAPTCHA.'));
                }
            } else {
    ...
    public function accountActivation($token=null){
        ...
        if(!empty($user)){
            $this->User->updateAll(
                array('User.is_active' => 1,'User.activation_token'=>1),
                array('User.id' => $user['User']['id'])
            );
            $this->Session->setFlash(__('Your account has been activated successfully, you can login now.'));
            $this->redirect(array('controller'=>'Users','action'=>'login'));    
        }else{
            $this->Session->setFlash(__('This link has been used before.'));
            $this->redirect(array('controller'=>'Users','action'=>'login'));   
        }
    }
                ...
                if(!empty($user)) { 
                    if($user['User']['is_deleted'] == 0){
                        if($user['User']['is_active'] == 1){
                            ...
                            $this->Session->setFlash(__('Your account is not activated, please activate your account by click on the activation link provided in registration email.'));
                        }
                    } else {
                       $this->Session->setFlash(__('Account no longer exists.'));
                    }
                }else {
                   $this->Session->setFlash(__('Invalid email or password, please try again.'));
                }
