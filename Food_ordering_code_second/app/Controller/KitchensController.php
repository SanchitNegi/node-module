      ...
      if (!empty($this->request->data)) {
         ...
         $this->Session->write('KitchenSearchData',json_encode($this->request->data));
          ...
          if (!empty($this->request->data['Order']['keyword'])) {
             $value = explode(" ",trim($this->request->data['Order']['keyword']));
           // $tags_trimmed = preg_replace('/\s+/', '', $this->request->data['Order']['keyword']);
            $criteria .= " AND (Order.order_number LIKE '%" . $value[0] . "%' OR User.fname LIKE '%" . $value[0] . "%' OR User.lname LIKE '%" . $value[0] . "%' OR User.email LIKE '%" . $value[0] . "%' OR DeliveryAddress.phone LIKE '%" . $value[0] . "%')";
          }
