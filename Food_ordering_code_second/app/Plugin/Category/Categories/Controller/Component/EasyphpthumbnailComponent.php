...
class EasyphpthumbnailComponent extends Component{
			...
			switch($this->size[2]) {
				...
				case 6:
					if (imagetypes() & IMG_WBMP) {
						$this->log('BMP support');
						$this->im=imagecreatefromwbmp($this->image);
						return true;
					} else {
						$this->log('No BMP support');
						return false;
					}
					break;
				default:
	...
	private function savethumb() {
		...
		if ($this->Thumbsaveas!='') {
			switch (strtolower($this->Thumbsaveas)) {
				...
				case "png":
					...
					break;
				case "bmp":
					$this->image=substr($this->image,0,strrpos($this->image,'.')).".bmp";
					$this->size[2]=6;
					break;
			}
		}
		...
		switch($this->size[2]) {
			...
			case 3:
				...
				break;			
			case 6:
				imagewbmp($this->thumb,$this->Thumblocation.$this->Thumbprefix.basename($this->image));
				break;
		}		
	...
	private function displaythumb() {
		...
		switch($this->size[2]) {
			...
			case 3:
				...
				break;
			case 6:
				header("Content-type: image/bmp");imagewbmp($this->thumb);
				break;
		}
		imagedestroy($this->im);
		imagedestroy($this->thumb);
	...
	public function Createbase64($filename="unknown") {
		...
		switch($this->size[2]) {
			...
			case 3:
				...
				break;			
			case 6:
				$encoding='data:image/bmp;base64,';
				imagewbmp($this->thumb);
				break;
		}
		...
		if (file_exists($this->Displacementmap[1])) {
			...
			switch($size[2]) {
				...
				case 3:
					...
					break;
				case 6:
					if (imagetypes() & IMG_WBMP) {$map=imagecreatefromwbmp($this->Displacementmap[1]);} else {$map=imagecreatetruecolor(100,100);}
					break;
				default:
