        ...
        if($this->request->is('ajax')){
            ...
            if($productInfo){
                ...
                if($productInfo['ItemPrice']){
                    $querySize = $productInfo['ItemPrice'][0]['size_id'];
                    $price_array=array();
                ...
                if($querySize == 0){
                    ...
                    $offer_result = $this->Offer->offerOnItemSize($itemId,$querySize);
                }
                if($offer_result){
                    $itemQuantity = $this->Session->read('Order.Item.quantity');
                    if($itemQuantity == $offer_result['Offer']['unit']){
                ...
                if($checked==1){
                   ...
                   //--------Here we are making calcualtion for topping------------------//
                    if($this->Session->check('Order.Item.topping_total')){
                        ...
                        $topping_total=round($topping_total, 2);
    ...
    public function addQuantity(){
        ...
        if($this->request->is('ajax')){
            ...
            if($offer_flag == 1){
                $this->loadModel('Offer');
                $this->loadModel('OfferDetail');
                if(@$present_item['Item']['size_id']){
                    $offer_result = $this->Offer->offerOnItemSize($present_item['Item']['id'],$present_item['Item']['size_id']);
                } else {
