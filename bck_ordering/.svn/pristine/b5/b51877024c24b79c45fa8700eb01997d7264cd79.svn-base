...
class PaymentsController extends AppController {
   ...
   public function beforeFilter() {
            // echo Router::url( $this->here, true );die;
             parent::beforeFilter();
             $adminfunctions=array('paymentList');
   ...
   public function orderSave($data=null,$paymemt_type=null){
      ...
      //if($data){
          ...
          if(AuthComponent::User()){
            ...
            if($sucess){
                $payment['OrderPayment']['id'] = $this->Session->read('Cart.payment_id');;
                $payment['OrderPayment']['order_id'] = $last_id;
                $this->OrderPayment->savePayment($payment);
