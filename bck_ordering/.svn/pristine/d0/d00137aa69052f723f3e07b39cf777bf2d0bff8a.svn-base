...
class StoreAvailability extends AppModel { 
    ...
    public function getStoreInfoForDay($selected_day=null,$storeId=null){
        if($storeId){
            $store_closedDay = $this->find('first',array('fields'=>array('day_name','start_time','end_time'),'conditions'=>array('StoreAvailability.store_id'=>$storeId,'StoreAvailability.day_name' =>$selected_day,'StoreAvailability.is_deleted' =>0,'StoreAvailability.is_active' =>1,'StoreAvailability.is_closed'=>0)));		
            return $store_closedDay;
        }     
     }
